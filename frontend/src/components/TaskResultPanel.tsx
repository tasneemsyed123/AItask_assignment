...
export function TaskResultPanel({
  ...
  if (status === 'FAILED') {
    return (
      <div className="rounded-xl bg-red-50 ring-1 ring-red-100 px-4 py-3">
        <p className="text-[11px] font-mono uppercase tracking-wide text-red-500 mb-1">Error</p>
  ...
  return (
    <div className="rounded-xl bg-emerald-50/60 ring-1 ring-emerald-100 overflow-hidden">
      <div className="flex items-center justify-between px-4 py-2 border-b border-emerald-100">
        <span className="text-[11px] font-mono uppercase tracking-wide text-emerald-600">
          Result{pretty ? ' · json' : ''}
        </span>
        <button
          onClick={handleCopy}
      ...
      <pre className="px-4 py-3 text-[12.5px] leading-relaxed text-[#1A1325] whitespace-pre-wrap break-words max-h-64 overflow-y-auto font-mono">
        {display}
      </pre>
    </div>
  );
